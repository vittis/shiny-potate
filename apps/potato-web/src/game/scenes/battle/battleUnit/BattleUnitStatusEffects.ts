...
export class BattleUnitStatusEffects extends Phaser.GameObjects.Container {
	...
	addStatusEffect({ name, quantity }: any) {
		const statusEffectAlreadyExists = this.statusEffects.find(
		...
		const statusEffect = this.scene.add.image(
			0,
			0,
			"statusEffect_" + name.toLowerCase().replace(/\s/g, "_"),
		);
	...
	removeStatusEffect({ name, quantity }: any) {
		const statusEffectToRemove = this.statusEffects.find(
			statusEffect => statusEffect.name === name,
		);
	...
	repositionStatusEffect() {
		...
		const frontColAmount = breakCol
			? Math.round(this.statusEffects.length / 2)
			: this.statusEffects.length;
