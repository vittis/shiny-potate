...
export class AbilityManager {
	...
	applyCooldownModifierFromMods(mods: Mod<MOD_TYPE.GRANT_BASE_STAT>[]) {
		this.activeAbilities.forEach(({ ability }) => {
			...
			const attackCooldownMods = mods.filter(mod => mod.payload.stat === STAT.ATTACK_COOLDOWN)

			const spellCooldownMods = mods.filter(mod => mod.payload.stat === STAT.SPELL_COOLDOWN)
